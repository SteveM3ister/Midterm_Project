...
ologit_result<-read.table("result.txt",header=TRUE)
```
...
ggplot(aes(x = name, y=X50.,
           ymin =X2.5., ymax = X97.5.),
       data = randominterval[1:3,]) + geom_pointrange(size=0.2,color="blue") +
    labs(x= "Predictors",
         y = "95% Confidence Interval",title="Coefficient Estimates (Intercept)")+
  theme(axis.text.x = element_text( family = "myFont" ,angle = 60))+geom_text(aes(label=round(X50.,2)),vjust=-1.5,size=3)
ggsave("interval_itc2.jpg",height=2.5,width=6)

randominterval['name']<-c("2|3","3|4","4|5","Government\n - Combination\n Government &\n Voluntary","Government\n - Local","Government\n - State/ County","Non - Profit\n Other","Non - Profit\n Private","Non - Profit\n Religious","Proprietary")
ggplot(aes(x = name, y=X50.,
           ymin =X2.5., ymax = X97.5.),
       data = randominterval) + geom_pointrange(size=0.2,color="blue") +
    labs(x= "Predictors",
         y = "95% Confidence Interval",title="Coefficient Estimates (Random Intercept)")+
  theme(axis.text.x = element_text( family = "myFont" ))+geom_text(aes(label=round(X50.,2)),vjust=-1.5,size=3)+
ggsave("interval_random.jpg",height=3,width=6)
```
...
mod <- stan_model("ordered_logistic_regression.stan")
...
pred_ologit<-rstan::sampling(mod,data=sch_data_pred,iter=4000,chains=2,control = list(adapt_delta = 0.9,max_treedepth = 15),core=4,seed=12345)

```
```{r}
 y_pred_samps<-rstan::extract(temp_ologit,pars="y_new")[[1]]
y_pred = matrix(NA,ncol=4,nrow= nrow(y_pred_samps))

for (i in 1:ncol(y_pred_samps)){
  probs = c(mean(y_pred_samps[,i] == 1), mean(y_pred_samps[,i] == 2), mean(y_pred_samps[,i] == 3),mean(y_pred_samps[,i] == 4))
  y_pred[i,]<-probs
}
y_pred%<>%data.frame()


n_sims<-nrow(y_pred_samps)
subset<-sample(n_sims,2000)
yrep<-as.data.frame(lapply(data.frame(y_pred_samps[subset,]),as.numeric))
ppcplot<-ppc_bars(as.numeric(fitset$Rating),as.matrix(yrep))
```
