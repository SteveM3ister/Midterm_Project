...
m2<-stan_polr(Rating~TimelyManner+DrugsTeaching+RiskofFalling+Depression+FluShot+Vaccine+FootCare+Walking+Bed+Bathing+Breathing+Operation+DrugsTaking+HospitalAdmitted+UnplannedCare+SkinIntegrity+TimelyMedication,data=fitset,prior=R2(0.3,"mean"))
...
##Validation
```{r}
# pr1<-model.matrix(~ordered(Rating), data=fitset[rownames(fitted(m3)),])-fitted(m3)
# pred1=fitted(m3)
# par(mfrow=c(2,3))
# for(i in 1:4) binnedplot(pred1[,i],pr1[,i])
predy<-posterior_predict(m2)
residr<-dcast(fitset,TimelyManner+DrugsTeaching+RiskofFalling+Depression+FluShot+Vaccine+FootCare+Walking+Bed+Bathing+Breathing+Operation+DrugsTaking+HospitalAdmitted+UnplannedCare+SkinIntegrity+TimelyMedication~Rating,value.var="freq"[,3:5]-predy)
